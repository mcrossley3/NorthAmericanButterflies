...
#########################################################################
# Get total abundance trends (aggregating all species per site)
# 100 km scale

# Import butterfly data
butterfly_counts1 = read.table('./data/butterfly_data_gridded_100km_1993-2018_w0s.txt',sep='\t',as.is=T,check.names=F,header=T)
butterfly_circles = spTransform(readOGR(dsn='./shapefiles/butterfly_sites_100km.shp',layer='butterfly_sites_100km',verbose=F,stringsAsFactors=F),CRS(proj1)) #shapefile with butterfly count circles
butterfly_grid = readOGR(dsn='./shapefiles/butterfly_grid_100km.shp',layer='butterfly_grid_100km',verbose=F,stringsAsFactors=F) #shapefile with grid
grid100 <- as(butterfly_grid, "sf")
g100 <- inla.read.graph('./shapefiles/nb100.graph') #graph network of grid neighbors
...
for (s in 1:length(sites)){
	...
	for (y in 1:length(years)){
		...
		N.species = length(unique(sdat2$Species[which(sdat2$N.butterflies>0)]))
		N.butterflies = sum(sdat2$N.butterflies)
...
write.table(butterfly_counts,'butterfly_total_abundance.txt',sep='\t',row.names=F)
...
form1 <- N.butterflies ~ -1 + # remove grand mean
  ...
  f(alpha_i, model="besag", graph=g100, constr=FALSE, scale.model=TRUE, hyper = list(prec = list(prior = "pc.prec", param = c(1, 0.01)))) +
  # cell ICAR random effort slopes
  f(eps_i, ln.Party_Hours, model="besag", graph=g100, constr=FALSE, scale.model=TRUE, hyper = list(prec = list(prior = "pc.prec", param = c(1, 0.01)))) +
  # cell ICAR random year slopes
  f(tau_i, std_Year, model="besag", graph=g100, constr=FALSE, scale.model=TRUE, hyper = list(prec = list(prior = "pc.prec", param = c(1, 0.01)))) +
...
ggsave('Allspecies_pit_1993-2018_w0s.png',plot=pit2,device='png',path='./plots/PIT/',width=8,height=8,units="in",dpi=300)
...
write.table(results_cells@data,'./inla_model_output/Allspecies_trends_1993-2018_w0s.txt',sep='\t',quote=F,row.names=F) # Write model output to file
res_sf <- as(results_cells, "sf")

# grid
grid2 <- merge(grid100, circles_per_cell, all=T)
...
ggsave('Allspecies_effort_1993-2018_w0s.png',plot =eps_p1,device='png',path='./plots/model_maps/1993-2018_w0s/',width=8,height=8,units="in",dpi=300)
ggsave('Allspecies_change_per_year_1993-2018_w0s.png',plot =tau_p1,device='png',path='./plots/model_maps/1993-2018_w0s/',width=8,height=8,units="in",dpi=300)
ggsave('Allspecies_relative_abundance_1993-2018_w0s.png',plot =alph_p1,device='png',path='./plots/model_maps/1993-2018_w0s/',width=8,height=8,units="in",dpi=300)
...
legend('topright',legend=c('No. sites = 497','No. cells = 295'),bty='n',cex=2,ncol=1)
...
# Make cell level maps
results_cells <- merge(butterfly_grid, post_sum, by="grid_id", all=F)
write.table(results_cells@data,'./inla_model_output/50km_wNAs/Allspecies_trends_1993-2018_50km_wNAs.txt',sep='\t',quote=F,row.names=F) # Write model output to file
res_sf <- as(results_cells, "sf")
...
# print cell maps
print(noquote('---drawing maps---'))
ggsave('Allspecies_effort_1993-2018_50km_wNAs.png',plot =eps_p1,device='png',path='./plots/model_maps/50km/',width=8,height=8,units="in",dpi=300)
ggsave('Allspecies_change_per_year_1993-2018_50km_wNAs.png',plot =tau_p1,device='png',path='./plots/model_maps/50km_wNAs/',width=8,height=8,units="in",dpi=300)
ggsave('Allspecies_relative_abundance_1993-2018_50km_wNAs.png',plot =alph_p1,device='png',path='./plots/model_maps/50km_wNAs/',width=8,height=8,units="in",dpi=300)

res_sf = read.table('./inla_model_output/50km_wNAs/Allspecies_trends_1993-2018_50km_wNAs.txt',sep='\t',as.is=T,check.names=F,header=T)
par(oma=c(0,0,0,0),mar=c(0,5,0,0))
boxplot(res_sf$tau,ylab='Total abundance trend',ylim=c(-10,10),cex.lab=2,boxlwd=2,staplelwd=2,whisklwd=2,whisklty=1,col='grey40',outpch=16,frame=F,yaxt='n')
axis(2,lwd=3,at=seq(-10,10,2),cex.axis=1.5)
abline(a=0,b=0,lwd=2,col='red',lty=2)
...
# Make cell level maps
results_cells <- merge(butterfly_grid, post_sum, by="grid_id", all=F)
write.table(results_cells@data,'./inla_model_output/50km_wNAs/Allspecies_trends_1993-2018_50km_wNAs_noTop4.txt',sep='\t',quote=F,row.names=F) # Write model output to file
res_sf <- as(results_cells, "sf")
...
ggsave('Allspecies_effort_1993-2018_50km_wNAs_noTop4.png',plot =eps_p1,device='png',path='./plots/model_maps/50km/',width=8,height=8,units="in",dpi=300)
ggsave('Allspecies_change_per_year_1993-2018_50km_wNAs_noTop4.png',plot =tau_p1,device='png',path='./plots/model_maps/50km_wNAs/',width=8,height=8,units="in",dpi=300)
ggsave('Allspecies_relative_abundance_1993-2018_50km_wNAs_noTop4.png',plot =alph_p1,device='png',path='./plots/model_maps/50km_wNAs/',width=8,height=8,units="in",dpi=300)

res_sf = read.table('./inla_model_output/50km_wNAs/Allspecies_trends_1993-2018_50km_wNAs_noTop4.txt',sep='\t',as.is=T,check.names=F,header=T)
par(oma=c(0,0,0,0),mar=c(0,5,0,0))
boxplot(res_sf$tau,ylab='Total abundance trend',ylim=c(-10,10),cex.lab=2,boxlwd=2,staplelwd=2,whisklwd=2,whisklty=1,col='grey40',outpch=16,frame=F,yaxt='n')
axis(2,lwd=3,at=seq(-10,10,2),cex.axis=1.5)
abline(a=0,b=0,lwd=2,col='red',lty=2)
...
res_sf1 = read.table('./inla_model_output/50km_wNAs/Allspecies_trends_1993-2018_50km_wNAs.txt',sep='\t',as.is=T,check.names=F,header=T)
res_sf2 = read.table('./inla_model_output/50km_wNAs/Allspecies_trends_1993-2018_50km_wNAs_noTop4.txt',sep='\t',as.is=T,check.names=F,header=T)
