...
sum(butterfly_counts$N.butterflies,na.rm=T)
dim(butterfly_counts)
...
for (i in 1:length(species)){
	...
	sp_counts = butterfly_counts[which(butterfly_counts$Species==species[i]),]
	
	# Remove sites with all zeroes
	sp_sites = unique(sp_counts$Site)
	site.counts = apply(array(sp_sites),1,function(x){length(which(sp_counts$N.butterflies[which(sp_counts$Site==x)]>0))})
	...
	sp.summary$Total.N.butterflies[i] = sum(sp_counts$N.butterflies)
...
#	ggsave(paste0(species[i],'_pit.png'),plot=pit2,device='png',path='./plots/PIT/',width=8,height=8,units="in",dpi=300)
	ggsave(paste0(species[i],'_pit_1993-2018_w0s.png'),plot=pit2,device='png',path='./plots/PIT/',width=8,height=8,units="in",dpi=300)
	...
	write.table(results_cells@data,paste0('./inla_model_output/',species[i],'_trends_1993-2018_w0s.txt'),sep='\t',quote=F,row.names=F) # Write model output to file
...
##	ggsave(paste0(species[i],'_change_per_year_significant.png'),plot =tau_p3,device='png',path='./plots/model_maps/',width=8,height=8,units="in",dpi=300)
	ggsave(paste0(species[i],'_effort_1993-2018_w0s.png'),plot =eps_p1,device='png',path='./plots/model_maps/100km/effort/',width=8,height=8,units="in",dpi=300)
	ggsave(paste0(species[i],'_change_per_year_1993-2018_w0s.png'),plot =tau_p1,device='png',path='./plots/model_maps/100km/change_per_year/',width=8,height=8,units="in",dpi=300)
	ggsave(paste0(species[i],'_relative_abundance_1993-2018_w0s.png'),plot =alph_p1,device='png',path='./plots/model_maps/100km/relative_abundance',width=8,height=8,units="in",dpi=300)
	
}
write.table(sp.summary,'./butterfly_trends_summary_1993-2018_w0s_100km.txt',sep='\t',quote=F,row.names=F)
sp.summary = read.table('./butterfly_trends_summary_1993-2018_w0s_100km.txt',sep='\t',as.is=T,check.names=F,header=T)
hist(sp.summary$mdn_Tau,breaks=seq(-80,240,5),main=NULL,xlab='Median change in abundance (%/year)',ylab='No. species')


##########################################################################################################################################################
# 50 km scale, pseudoabsences reported as NA

# Import butterfly data
butterfly_counts = read.table('./data/butterfly_data_gridded_50km_1993-2018_wNAs.txt',sep='\t',as.is=T,check.names=F,header=T)
length(unique(butterfly_counts$grid_id))
gcount = apply(array(unique(butterfly_counts$grid_id)),1,function(x){length(unique(butterfly_counts$Site[which(butterfly_counts$grid_id==x)]))})
mean(gcount)
sd(gcount)/sqrt(length(gcount))
summary(gcount)
...
for (i in 1:length(species)){
	...
	#skipped i=174 "Decinea percosius" - has only one year with data
	...
	# Subset data for species i
	sp_counts = butterfly_counts[which(butterfly_counts$Species==species[i]),]
#	print(length(which(is.na(sp_counts$N.butterflies))))
...
#	sum(out1$cpo$failure, na.rm=T) -2 * sum(log(out1$cpo$cpo[out1$cpo$failure==0]), na.rm=T)
	...
	pit2 <- ggplot(data=pit1, aes(x=PIT)) +
	  ...
	  xlab("Probability integral transform (PIT)") +
	  ylab("Count"); pit2; summary(pit1$PIT)
	ggsave(paste0(species[i],'_pit_1993-2018_50km_wNAs.png'),plot=pit2,device='png',path='./plots/PIT/',width=8,height=8,units="in",dpi=300)
	...
	# Make cell level maps
	results_cells <- merge(butterfly_grid, post_sum, by="grid_id", all=F)
	write.table(results_cells@data,paste0('./inla_model_output/50km_wNAs/',species[i],'_trends_1993-201_50km_wNAs.txt'),sep='\t',quote=F,row.names=F) # Write model output to file
	res_sf <- as(results_cells, "sf")
...
##	ggsave(paste0(species[i],'_change_per_year_interval.png'),plot =tau_p2,device='png',path='./plots/model_maps/',width=8,height=8,units="in",dpi=300)
##	ggsave(paste0(species[i],'_change_per_year_significant.png'),plot =tau_p3,device='png',path='./plots/model_maps/',width=8,height=8,units="in",dpi=300)
	ggsave(paste0(species[i],'_effort_1993-2018_50km_wNAs.png'),plot =eps_p1,device='png',path='./plots/model_maps/50km_wNAs/effort/',width=8,height=8,units="in",dpi=300)
	ggsave(paste0(species[i],'_change_per_year_1993-2018_50km_wNAs.png'),plot =tau_p1,device='png',path='./plots/model_maps/50km_wNAs/change_per_year/',width=8,height=8,units="in",dpi=300)
	ggsave(paste0(species[i],'_relative_abundance_1993-2018_50km_wNAs.png'),plot =alph_p1,device='png',path='./plots/model_maps/50km_wNAs/relative_abundance/',width=8,height=8,units="in",dpi=300)
	
}
write.table(sp.summary,'./butterfly_trends_summary_1993-2018_50km_wNAs.txt',sep='\t',quote=F,row.names=F)
