...
#	ggsave(paste0(species[i],'_pit.png'),plot=pit2,device='png',path='./plots/PIT/',width=8,height=8,units="in",dpi=300)
	...
	write.table(results_cells@data,paste0('./',species[i],'_trends_1993-2018_w0s.txt'),sep='\t',quote=F,row.names=F) # Write model output to file
...
#	sum(out1$cpo$failure, na.rm=T) -2 * sum(log(out1$cpo$cpo[out1$cpo$failure==0]), na.rm=T)
	...
	# Make cell level maps
	results_cells <- merge(butterfly_grid, post_sum, by="grid_id", all=F)
	write.table(results_cells@data,paste0('./',species[i],'_trends_1993-201_50km_wNAs.txt'),sep='\t',quote=F,row.names=F) # Write model output to file
	res_sf <- as(results_cells, "sf")
...
##	ggsave(paste0(species[i],'_change_per_year_interval.png'),plot =tau_p2,device='png',path='./plots/model_maps/',width=8,height=8,units="in",dpi=300)
##	ggsave(paste0(species[i],'_change_per_year_significant.png'),plot =tau_p3,device='png',path='./plots/model_maps/',width=8,height=8,units="in",dpi=300)
	ggsave(paste0(species[i],'_effort_1993-2018_50km_wNAs.png'),plot =eps_p1,device='png',path='./',width=8,height=8,units="in",dpi=300)
	ggsave(paste0(species[i],'_change_per_year_1993-2018_50km_wNAs.png'),plot =tau_p1,device='png',path='./',width=8,height=8,units="in",dpi=300)
	ggsave(paste0(species[i],'_relative_abundance_1993-2018_50km_wNAs.png'),plot =alph_p1,device='png',path='./',width=8,height=8,units="in",dpi=300)
